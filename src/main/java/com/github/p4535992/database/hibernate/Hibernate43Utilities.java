    ...
    public Interceptor createInterceptor(Class<? extends Interceptor> interceptorClass){
        try {
            try {
                ...
                throw new InstantiationException(e.getMessage(),Hibernate43Utilities.class,e);
            }
...
//                javax.persistence.Persistence.createEntityManagerFactory("YOUR PU");
    ...
    /**
     * href : http://myjourneyonjava.blogspot.it/2014/12/different-ways-to-get-connection-object.html
     * href: http://stackoverflow.com/questions/3526556/session-connection-deprecated-on-hibernate
     * @param session the {@link Session} Hibernate where is allocate dthe Connection.
     * @return the {@link Connection} SQL to the database.
     */
    ...
    public Connection getConnectionOLDHibernateWithSpringReflect(Session session,Method connectionMethod) {
        try {
            if (connectionMethod == null) {
                // reflective lookup to bridge between Hibernate 3.x and 4.x
                connectionMethod = session.getClass().getMethod("connection");
            }
            return (Connection) org.springframework.util.ReflectionUtils.invokeMethod(connectionMethod, session);
        }
